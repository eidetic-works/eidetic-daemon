...
| Tool | Args | Returns |
|---|---|---|
| `query_engrams` | `surface` (required), `limit` (default 50, cap 500), `since` (unix ns, default 0), `before` (unix ns, default 0, v0.0.21+), `raw_chunks` (bool, default false) | JSON array of engrams ordered by timestamp descending. `since`+`before` define a half-open time window. Chunked records (per ADR-018) are reassembled by default; `raw_chunks=true` returns chunks as separate engrams. |
